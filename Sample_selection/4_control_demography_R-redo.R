###############################Asthma control selection###########################

# This script is used to explore the demographic of the UKBiobank data 
# for control selection, as well as extracting them for PLINK.
...
setwd("/Users/christianarenasdaza/Desktop/lab_ch2/input/4_Control_selection_ch")


#####  Load files 

# my data UKBiobank
samples_subset <- read.table(file = paste("column_subset_AST_ch_sh.tab", sep = ""),
                             header=TRUE, 
                             sep="\t")

# Patient that had widrew from the UKBiobank
...
# File with the RA patient samples: 
AST_subset_sm <- read.table(file = paste("samples_AST_stric_sm.tab", sep = ""),
                         header=TRUE,
                         sep="\t")



#  ALLparticipant with inclamatory conditions 
IC_subset <- read.table(file = paste("samples_demography_IC.tab", sep = ""),
                        header=TRUE, 
                        sep="\t")
...
samples_subset$Moderate_excercise <- paste(samples_subset$f.884.0.0, 
                                          samples_subset$f.884.1.0,
                                          samples_subset$f.884.2.0,
                                          samples_subset$f.884.3.0)
...
# Make a data frame with just the desire columns
samples_subset_demography<- samples_subset[, c(399:408)]

# Delete all the NA from the columns 
samples_subset_demography$Smoking <- gsub("NA", "",  samples_subset_demography$Smoking)
samples_subset_demography$Moderate_excercise <-  gsub("NA", "", samples_subset_demography$Moderate_excercise)

# Formatting the  smooking column
# -3 = missing data for the smooking column
samples_subset_demography$Smoking <- gsub(" ", "", samples_subset_demography$Smoking)  # Remove white spaces
samples_subset_demography$Smoking <- str_extract(samples_subset_demography$Smoking, "^[0,1,2]") # Take the first code 
samples_subset_demography$Smoking <- gsub("^$", "-3", samples_subset_demography$Smoking) # fill missing data 
samples_subset_demography$Smoking <- replace_na(samples_subset_demography$Smoking, "-3") # replace NA


# Format moderate excercise column
# In this column -3 and -1 means missing data 
samples_subset_demography$Moderate_excercise <- gsub(" ", "", samples_subset_demography$Moderate_excercise) # Remove white spaces
samples_subset_demography$Moderate_excercise <- gsub("-3", "-9", samples_subset_demography$Moderate_excercise) # replace missing data
samples_subset_demography$Moderate_excercise <- gsub("-1", "-9", samples_subset_demography$Moderate_excercise) # replace missing data
samples_subset_demography$Moderate_excercise  <- str_extract(samples_subset_demography$Moderate_excercise , "(?![-]?[9])([+]?[0-7])") # Take the first code 
samples_subset_demography$Moderate_excercise  <- replace_na(samples_subset_demography$Moderate_excercise, "-9") # replace missing data

# Delete all samples that have inflammatory conditions
samples_subset_demography <- subset(samples_subset_demography, !(id %in% patients_witdraw$ID))
samples_subset_demography <- subset(samples_subset_demography, !(id %in% AST_subset_sm$id))
samples_subset_demography <- subset(samples_subset_demography, !(id %in% IC_subset$id))
...
# sub  setting for age, ethnicity and chromosome information


samples_subset_demography_final <- subset(samples_subset_demography, samples_subset_demography$age >= 52 & 
...
##### Save files to your desired directory


# Save control samples: 
write.table(samples_subset_demography_final, 
            file = paste("/Users/christianarenasdaza/Desktop/lab_ch2/output/4_Control_selection_ch/controls_demography_no_IC.tab", sep = ""),
            sep = "\t",
            quote = FALSE,
            row.names = FALSE)  
...
png(file = paste("/Users/christianarenasdaza/Desktop/lab_ch2/output/4_Control_selection_ch/sex_Controls_no_IC.png", sep = ""), width = 900, height = 600)

sex_plot

dev.off()  

png(file = paste("/Users/christianarenasdaza/Desktop/lab_ch2/output/4_Control_selection_ch/age_Controls_no_IC.png", sep = ""), width = 900, height = 600)

age_plot

dev.off()  

png(file = paste("/Users/christianarenasdaza/Desktop/lab_ch2/output/4_Control_selection_ch/normal_ethnicity_Controls_no_IC.png", sep = ""), width = 1300, height = 1100)

normal_ethnicity

dev.off()  

png(file = paste("/Users/christianarenasdaza/Desktop/lab_ch2/output/4_Control_selection_ch/log_ethnicity_Controls_no_IC.png", sep = ""), width = 1300, height = 1100)

log_ethnicity

dev.off()  

png(file = paste("/Users/christianarenasdaza/Desktop/lab_ch2/output/4_Control_selection_ch/addictions_Controls_no_IC.png", sep = ""), width = 900, height = 600)
...
############################### Dempgraphic for Cacusassian Controls ##########
...
sex_cg <- data.frame(column = c("sex", "sex"), 
                  sex = c("Male", "Female"),
                  proportions = c((nrow(samples_subset_demography_final[samples_subset_demography_final$sex == 1, ])/nrow(samples_subset_demography_final))*100,
                                  (nrow(samples_subset_demography_final[samples_subset_demography_final$sex == 0, ])/nrow(samples_subset_demography_final))*100))

sex_cg$proportions <- round(sex_cg$proportions, 0)


sex_cg_plot <- ggplot(data = sex_cg, aes(x = column, y = proportions, fill = sex)) +
  geom_bar(stat = "identity", colour = "black", size = 2) +
  scale_fill_manual(values=c('brown1','deepskyblue3')) + 
...
png(file = paste("/Users/christianarenasdaza/Desktop/lab_ch2/output/4_Control_selection_ch/sex_Controls_ATSHMA_casucassian.png", sep = ""), width = 900, height = 600)
...
age_plot_cg<- ggplot(samples_subset_demography_final, aes(x = age, fill = as.factor(sex))) + 
  geom_density(alpha = 0.5) + 
  labs(x = "Age", y = "Density") + 
  scale_fill_manual(values=c('brown1', 'deepskyblue3'), labels =  c("Female", "Male")) + 
  theme(axis.title = element_text(size = 20),
...
png(file = paste("/Users/christianarenasdaza/Desktop/lab_ch2/output/4_Control_selection_ch/age_Controls_ATSHMA_casucassian.png", sep = ""), width = 900, height = 600)
